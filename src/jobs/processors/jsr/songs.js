...
import { SongJSR } from '../../../models/SongModel.js';
import { readFile } from '../../../utils/fileUtil.js';


const __dirname = dirname(fileURLToPath(import.meta.url));

const processJSRSongs = async () => {
  let songSaveCount = 0;
  const songsFilePath = path.join(__dirname, '..', '..', '..', 'storage', 'songs', 'jsr-audio.json');
  const songs = await readFile(songsFilePath);
  const parsed = JSON.parse(JSON.stringify(songs));
  console.log(`Found ${JSON.parse(parsed.toString()).length} JSR songs to process`);

  for (const rawSong of JSON.parse(parsed.toString())) {
    const audioLink = rawSong;

    const names = getNames(audioLink);
    const artistId = await getArtist(audioLink);
    const name = names[0];
    const shortName = names[1];
    const gameId = await BaseModel.getGameId('Jet Set Radio');

    const exists = await BaseModel.getByKeyAndValue('songJsr', 'name', name);
    if (!exists) {
      const song = new SongJSR();
      song.name = name;
      ...
      LOGGER.warn(`Found existing JSR Song with name ${name}`);
    }
  }

  LOGGER.info(`Saved ${songSaveCount} new JSR Songs`);
}
...
const saveArtist = async (artistName) => {
  ...
  if (!exists) {
    ...
    await updateArtist(exists);
    return exists;
  }
}

// Add JSR gameId to existing Artists
const updateArtist = async (artist) => {
  const gameIds = artist.gameIds;
  if (gameIds.length === 1) {
    const gameId = gameIds[0];
    const exists = await BaseModel.getById('game', gameId);
    if (exists.name === 'Jet Set Radio Future') {
      const jsrGameId = await BaseModel.getGameId('Jet Set Radio');
      artist.gameIds.push(jsrGameId);
      await artist.save();
      LOGGER.info(`Added JSR GameID to artist ${artist.name}`);
    }
  }
}

export default processJSRSongs;